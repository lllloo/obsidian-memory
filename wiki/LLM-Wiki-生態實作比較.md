...
- **值得補的缺口（2026-07-21 提出，2026-08-11 已解決）**：本 vault 的矛盾處理原只寫「就地標記矛盾」，**舊主張被推翻後的處置是隱性的**——agent 可能改寫、可能並列、可能默默刪掉，事後看不出來。llm-wiki-kit 的 explicit supersession 是對症的方向（雖其 `docs/supersession.md` 為 404、無可照抄的實作規格，且其 lint 明列 v0.1 out of scope），但**不必等它成熟**：2026-08-11 使用者拍板，於憲法檔（現為 `AGENTS.md`）寫入慣例第 6 條增補 supersession 標記——推翻既有主張時原句就地改標「已被取代（日期）：新結論見某處」並保留；只有「推翻」才標、不回頭挖舊頁補標。

...
- **日期漂移查無實例**——表面上散亂的多個拍板日期（07-10／07-17／07-20／07-21）經比對皆為**不同事件**，非同一件事被寫成不同日期。
- **數字跨頁重述確實存在**（`90.2` 見於 4 頁、`1,642` 見於 3 頁、「25 條」見於 6 頁），故漂移機制結構上成立；**但未查到任何實際不一致**。

**因此：機制可遷移，優先度不可遷移。** 對此規模且有交叉引用紀律的 vault，數值一致性是**潛在風險而非現存問題**——原先「投報比最高的補強方向」判斷過強，已撤回，未為此新增 lint 規則。

**仍成立的推論**：**自動矛盾偵測本質不可靠**（AUROC 75.1%，即隨機抽一對正／負樣本只有約四分之三能被正確排序；AUROC 不是分類正確率、不能換算成錯誤率，但離可用的偵測器仍遠）。本 vault「語意項一律自主修補」等於在該鑑別力的偵測器上直接執行寫入；不必改回只報告，但修補動作應在 diff 中可辨識。

**對本 vault 真正該管的不是數字準不準，是數字帶不帶限定詞。** 這是第 2 條而非本條的範疇——一個掉了限定詞的正確數字，比一個有限定詞的粗略數字危險得多，因為它讀起來完全可信。這兩件事機制不同、緩解手段也不同（前者靠一致性掃描，後者靠改寫時的寫作紀律），常被混為一談。
...
- 品質訊號的缺口：[[LLM-as-judge-知識庫頁面評分]]——本頁「無法驗證盲點」一節指出品質回饋全靠人眼看 diff，該頁調查自動化品質評分的可行性，結論是現成方案沒有為互聯知識庫設計者，且 LLM 自評受 self-preference bias 限制

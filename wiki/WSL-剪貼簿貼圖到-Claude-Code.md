...
把畫面內容交給 agent 看，是 [[設計品質的可量化檢測]] 那套「截圖 → AI 核對設計偏離」閉環的前置環節——在 WSL 上這一步會無聲斷掉，該頁的檢測流程也就跑不起來。反方向的載體問題見 [[AI-生成流程圖與架構圖]]：那頁處理「怎麼把圖給人看」（固化成 mermaid.live 連結），本頁處理「怎麼把圖給 agent 看」。

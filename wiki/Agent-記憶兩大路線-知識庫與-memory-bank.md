...
- [[Mem0]] — 本頁 A／B 分野在單一工具上的落地案例：拍板讓 mem0 只當「隨手記收件匣」、A 路線的嚴謹知識仍留 vault。**注意它不歸 B 路線**——B 是服務單一專案的 memory bank（固定角色檔、無獨立來源層、每次讀全部），mem0 則是跨專案跨機器的雲端記憶層；兩者共通處只有「非知識資產複利」這個否定性質，故本頁用「隨手記」而非路線歸屬描述它。其失效模式（抽取污染、幻覺自我複製）正是「什麼不該交給自動抽取」的反面教材。其失效模式（抽取污染、幻覺自我複製）正是「什麼不該交給自動抽取」的反面教材。

...
> 此為 Karpathy 原文對「LLM 執行簿記」這件事本身的主張，未經獨立效果驗證；agent 自主工作流整體（含此模式所屬的「沉澱回知識庫」一步）的已知風險見 [[AI-自主工作流的實證檢驗]]（記憶檔易被忽略、compaction 會靜默丟棄約束）與 [[LLM-Wiki-生態實作比較]]（大規模盲目編譯的高失敗率、自然語言禁令守門不可靠）。
...
- 交換格式邊界：[[OKF-與本-vault-的相容性]]——本模式與外部交換格式 OKF 的適用邊界：內部不遷移、保留為未來匯出 profile 的拍板在該頁。

...
- [[視覺回歸測試的落地與陷阱]] — B 路線在**畫面層**的分支：上一條展開的是行為層，而 review 一個 UI 改動時人真正在看的是版面有沒有壞，那正是本頁「verification tax」最貴的一段。該頁記下業界的現況——連專做視覺回歸的廠商都只主張把 flake 隔離、不主張消除，可視為 A 路線工具化承諾的一個對照樣本。
...
- [[平行-agent-產出的合併與-review]] — 多 agent 平行產出時的版本：衝突率、合併順序、best-of-N 挑選上限，以及一份企業研究觀測到的具體機制——PR 量增 3.1 倍、reviewer 只增 1.5 倍，缺口由 AI review 吸收、人工 review 退化成無留言放行。

...
Robert C. Martin 在 2026-07-23 的[推文](https://x.com/unclebobmartin/status/2080257779395154409)（5M views）宣告：**不讀 agent 寫的任何程式碼**，改用「極端約束」包圍 agent——「unit tests、gherkin tests、QA procedures、quality metrics、mutation testing、test coverage，以及其他一堆」，跑完這串 gauntlet 之後對產出有「very high confidence」。

這則推文值得逐項研究，原因不是它有名，而是**他把每一道閘門都自己寫成了工具、公開在 GitHub 上**。這讓「他到底在做什麼」從意見變成可檢驗的事實——本頁的一手依據幾乎全部來自那些 repo 的原始碼與 README，而不是轉述文章。本頁是 [[用測試約束-AI-產碼]] 的一個具體人物案例：那頁回答「哪些手段有證據」，本頁回答「一個把這條路線推到極致的人，實際搭出來的東西長什麼樣、哪裡撐不住」。
...
換句話說，他把 TDD 從「寫在規則檔裡的口號」搬成**角色的定義**，並且用另一個角色（`specifier`）先把規格固定下來。這恰好落在證據支持的那一邊：先固定驗證標的，而不是下流程命令。（**medium**：兩份 repo 文件為一手，但「這個設計選擇是有意識地回應 TDD 指示無效」是本頁的推論，非他本人陳述。）
...
`empire-2025` 的實作是同一管線的專案版：`.txt`（Given/When/Then）→ parser → `.edn` → generator → Speclj spec → 執行。`AGENTS.md` 對 agent 的規則裡有兩條特別值得抄：
...
這一項最容易被忽略，卻是理解整套主張的關鍵。在 `six-pack` 編制裡，QA 是一條完整的鏈：`specifier` 產出「end-to-end QA procedures」，最後 `QA` 角色「把 specifier 的 QA procedures 轉成可執行腳本，執行最終的使用者介面驗證」。

而他本人做什麼？據二手轉述（**未核一手推文**，見證據表），他 review Gherkin 驗收測試、review QA procedures（「有時徹底、有時抽查」），並且「週期性地做一次最終手動測試」。`empire-2025` 的 `--headless=N`、`--seed=N`、`--log`、`--debug-dump` 這幾個旗標，正是為了讓這種人工與自動驗證跑得動而建的。

**所以「不讀 AI 產的程式碼」這個標題是準確的，但推論成「他不看 agent 的產出」是錯的。** 正確的描述是：他把人類注意力從**實作層**整個搬到**規格層與行為層**——規格（Gherkin）他讀，QA 程序他讀，成品他親手玩。省下來的是讀函式本體的時間，不是驗證的時間。這與 [[AI-產碼加速下的-review-瓶頸]] 的四條路線是同一件事的兩種說法：他選了「約束前移」而非「改善 review」。
...
回讀 `engineering.prompt` 的 Guardrails 一節，這個質疑站得住：
...
這是**純粹的請求**。`empire-2025/AGENTS.md` 的「Never modify an acceptance test `.txt` file without explicit permission」同樣如此。對照 [[用測試約束-AI-產碼]] 第五節的五層防繞過，他只有第 1 層（寫進規則檔），沒有第 2 層（PreToolUse deny）、第 3 層（CI 鏡像）、第 4 層（CODEOWNERS 保護護欄本身）——`empire-2025` 有 `.github` 目錄，但整套驗證是本地 `clj -M:...` 指令鏈，不是 merge gate。

而該頁引 ImpossibleBench 的量測是：**GPT-5 在測試與規格矛盾時作弊率 76%，即使明確指示「發現測試有問題就停下」也未歸零**。「請不要手改 manifest」屬於同一類指示。@GeoffreyHuntley 在串裡的回應「we need to engineer the constraint's」正是這個缺口——約束要被**工程化**（機械上做不到），不是被**宣告**。
...
**建議的採用順序**（沿用 [[用測試約束-AI-產碼]] 第六節的「先量基線」原則）：CRAP 一行公式先算一次找出最該補測試的函式 → 已排定的 mutation 基線 → 若那時仍覺得測試在說謊，再考慮手寫 introverted test 偵測。Gherkin 那一整套除非專案規模到了他說的「larger projects」，否則按他自己的話就是 overloading。
...
| 護欄僅存在於 prompt 層，無機械強制 | **medium-high**：`engineering.prompt` 與 `AGENTS.md` 全文可讀、未見 CI gate；但無法排除他有未公開的本地機制 |
| empire-2025 屬規格封閉、行為可決定的領域 | **high**：README 與 AGENTS.md 直接可讀 |
| 他 review Gherkin 與 QA procedures、但不 review 單元測試 | **low-medium**：**二手轉述**（[explainx.ai](https://explainx.ai/blog/uncle-bob-ai-coding-gauntlet-tests-not-reviews-july-2026) 宣稱引自串內回覆），**未核一手推文**；與 `swarm-forge` 角色分工相容故未否決 |
| 「他把 TDD 從規則檔搬成角色定義是有意識回應 TDD 指示無效」 | **low**：本頁推論，非他本人陳述；repo 文件只支持「coder 角色明寫 with TDD」這個事實 |

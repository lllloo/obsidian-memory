...
[[用測試約束-AI-產碼]] 是「決策側」最乾淨的落地案例：要擋住 agent 繞過驗證的手法（`--no-verify`、skip 環境變數、改寫 hook 設定本身），需要的**恰好只有** allow/block 這一個能力，「內容過不去」在這個用途上完全不構成限制。該頁把它列為防繞過分層的第 2 層，並強調它只是其中一層——本頁的輸出契約分析解釋了為何這一層擋得住、也解釋了為何它擋不了「把規則本身改弱」那類繞過。

[[Agent-Skill-腳本路徑的規範與實況]] 是同一種「規範寫得到 ≠ 實作做得到」的另一個案例：該頁的落差不在輸出契約而在路徑解析——Agent Skills 規範要求 client 端把 skill 相對路徑改寫成絕對路徑，harness 未實作時裸相對路徑就會找不到檔。

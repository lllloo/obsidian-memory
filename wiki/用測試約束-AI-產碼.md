...
- [[不讀碼時該看哪些圖]] — 互補的另一軸：本頁的機械判準管**行為對不對**，該頁的管**東西有沒有放對地方**（架構邊界）。兩者都不可替代對方——依賴規則全綠不代表邏輯正確，本頁的測試全綠也不代表架構沒被侵蝕。該頁同樣落在「把判準外移成 CI 閘門」這條路上，可視為本頁第五節防繞過思路在結構維度的版本。
...
- [[視覺回歸測試的落地與陷阱]] — 本頁四種失效模式全部發生在**行為層**，畫面層是結構性空白：type check、lint 與單元測試都不斷言版面，而 AI 改 UI 碼動到的正是類別名與版面結構。該頁把視覺回歸的取捨收斂成「渲染環境的控制權放在哪」，並記下它自己的限制——VR 對邏輯正確性零覆蓋，與本頁的手段互補而非替代。
- [[AI-時代的後端語言選擇]] — 該頁查證「哪種後端語言讓 coding agent 產碼更好」後結論是評測排名矛盾、無語言優勢證據，判讀收斂到「有型別檢查與測試的快速回饋迴圈才是著力點」——正是本頁的主題，本頁是那條判讀的具體做法。

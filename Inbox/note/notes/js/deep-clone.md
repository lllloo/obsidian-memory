...
- `structuredClone` 支援多數結構化可複製型別並能處理循環參考；但遇到函式、閉包、Symbol、DOM 節點等無法序列化的值時，會拋出 `DataCloneError`（DOMException）使整個複製**失敗**，而非像 JSON 方法那樣靜默略過該屬性。不確定輸入內容時請用 try/catch 容錯。

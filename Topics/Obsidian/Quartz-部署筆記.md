...
- **發佈範圍**：`ignorePatterns`（定義在發佈 repo 的 `quartz.config.ts`）排除 `.obsidian`、整個 `Inbox/`（含 YouTube 與 Clippings）、根目錄治理檔（`CLAUDE.md`、`AGENTS.md`、`README.md`、`vault-map.md`、`SYSTEM-DESIGN.md`）與 CI checkout 後落在 `content/` 的工程檔；實際只發佈 `Cards/`、`Topics/` 與根 `index.md`。完整清單以發佈 repo 的 `quartz.config.ts` 為準。

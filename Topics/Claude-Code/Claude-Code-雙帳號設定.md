...
New-Item -ItemType SymbolicLink -Path "$HOME\.claude-p\jobs" -Target "$HOME\.claude\jobs"
```
...
`jobs` 也可以共用主帳號的工作紀錄；若第二帳號原本已經有 `jobs` 內容，先改名保留成 `jobs.local`，再補 symlink，避免直接覆蓋。

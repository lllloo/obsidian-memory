...
對 Obsidian vault 執行語意層稽核，回傳結構化 JSON。定位為「兜底層」：寫入路徑（write 流程等）依 `CLAUDE.md` 的寫入前 Checklist 自檢預防；deterministic 規則由 `scripts/vault-check.mjs` 處理；本流程抓漏網——Web Clipper / 手動編輯帶入的敏感資料與 schema 缺漏、跨筆記 emergent 問題（broken wikilinks、tag drift）。大多數時候結果應為空；非空代表寫入路徑或外部來源需要補強。

**不負責的範圍**：筆記位置（Inbox/Cards/Topics）屬於使用者主觀判斷，不在稽核清單，禁止產生此類建議。

## 唯讀工具契約（強制）

> 此契約與 `ob/references/query.md` 保持同步——任一修改，另一份同步更新。
...
- **不再呼叫其他 subagent**
- **path 一律正規化**：vault root 相對路徑（如 `Cards/foo.md`），不要絕對路徑
...
```bash
test -f master-index.md
```

失敗即直接輸出空結果，`error` 欄寫「cwd 不在 vault root，無法執行 audit」。orchestrator 通常會在 step 1 早停，這條是兜底，避免 subagent 被孤立呼叫時誤掃。

## 掃描範圍

- 全部 `**/*.md`
- **排除**：`.obsidian/`、`index.md`、`master-index.md`、`CLAUDE.md`

開工前先 `Read CLAUDE.md` 取得當前的「寫入前 Checklist」內容（敏感資料定義、frontmatter schema、tag 一致性判準、命名規則）作為稽核依據。Checklist 更新時本流程自動跟上，不需改 reference。
...
- target 對照 `**/*.{md,base}` 的檔名（Quartz `shortest` 語義，比 basename）
- 排除 code fence 內的 `[[...]]`
- 對每個斷鏈，找最相似的現有檔名做 suggestion；找不到就 `suggestion: null`

### 3. sensitive_data — 敏感資料

**清單以 `CLAUDE.md` 的「寫入前 Checklist §1」為準**（regex 白名單 + 自然語言密碼 + 個資 / 公司內部資訊）。開工前已 Read 該檔，依當下清單掃描，CLAUDE.md 更新時本流程自動跟上。
...
```json
{
  ...
  "broken_wikilinks": [
    {
      "file": "Topics/Claude-Code/index.md",
      "line": 42,
  ...
  "tag_conflicts": [
    {
      ...
      "files": ["Cards/a.md", "Topics/X/b.md"]
    }
  ],
  "error": null
}

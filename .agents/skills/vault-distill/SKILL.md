...
**僅當使用者明確指定了主題名稱**（例：「整合 Claude Code Skills 的筆記」、「distill 動效相關」）且沒下動詞時，才執行以下掃描：
...
- 掃 `Cards/` 是否有含「## 來源」章節但無 `moc` tag 的 .md 檔（疑似未補 tag 的 MOC）→ 若有，警示用戶確認再繼續
...
先 Read `.agents/skills/vault-distill/references/moc-structure.md` 確認骨架與「常見錯誤」checklist，再寫入 `Cards/<主題>.md`。
...
- `<MOC 絕對路徑>` → 實際路徑（e.g., `/Users/.../Cards/主題.md`）
...
3. 或掃描 MOC 現有 wikilink，把 `[[...]]` 指向 `Cards/` 或 `Inbox/` 的檔案視為候選

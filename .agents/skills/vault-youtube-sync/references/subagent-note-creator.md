...
- **不主動加 wikilink**：raw/ 是「消化完刪除」的暫存，筆記彼此 `[[wikilink]]` 沒意義（會一起被刪）。即使主題重疊也**不要**掃 `NOTES_DIR` 找兄弟筆記補連結——wikilink 長出來的時機是使用者把 Cards/ 歸檔到 Topics/，由人決定。例外：`parent: "[[01.index]]"` 是 schema 必填，照寫。
- **檔名命名**：
  - 繁體中文為主，技術名詞與品牌名保留英文

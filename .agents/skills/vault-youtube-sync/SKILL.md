...
grep "^last_sync_id:" "Inbox/YouTube/<頻道名>/01.index.md" 2>/dev/null | sed 's/last_sync_id: //'
```

**Checkpoint 過濾邏輯：**

- 若資料夾**不存在**或 `01.index.md` **無 `last_sync_id`**：全部影片都處理，建立資料夾 `mkdir -p "Inbox/YouTube/<頻道名>"`

...
Usage (cwd = repo root):
    ...
    # 一般用法：自動讀 feeds/updates/01.index.md
...
Sources come from feeds/updates/01.index.md by default.
...
DEFAULT_INDEX = "feeds/updates/01.index.md"


def parse_index(path: str) -> tuple[list[str], list[str], bool]:
    """Parse feeds/updates/01.index.md into (official, repos, starred).
...
def main() -> int:
    ...
    if not official_sources and not explicit_repos and not args.starred:
        print("ERROR:usage:no sources provided; pass --official, --repo, or --starred "
              "(driven by feeds/updates/01.index.md)")
        return 1

...
`raw/Updates/01.index.md` 是唯一來源真實值：追蹤哪些工具、抓哪些 changelog、是否啟用 starred 同步，都由此檔決定。不要在 skill 或腳本硬編碼工具清單。
...
- `--index <path>`：指定來源 index，預設 `raw/Updates/01.index.md`。
...
- 不把日報 wikilink 追加回 `raw/Updates/01.index.md`。

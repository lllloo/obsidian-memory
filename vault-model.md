...
同樣的時間迭代問題也會反過來咬筆記正文：把精確版本號釘死進 Cards / Topics，下一版就過期，於是「校對過時資訊」變成永遠追不完的循環。寫入時的原則：

- **描述行為怎麼變，而非釘死版本號**。寫「近期版本起改成 opt-in」勝過「v0.10.0 起」；確切切換版本指向官方 release notes / changelog，由讀者回查。
- **精確版本號只在它是該行為的關鍵分水嶺、且讀者必須知道時才留**，並標註查證來源。
- 與版本耦合度低的**行為約束仍要留**（如某功能需特定 model 或 API），別連這些一起淡化掉。

道理同上：筆記留「我理解的、之後可更新的版本」，易變細節交給官方文件。
...
前提是這份原料**使用者本人已讀過／看過**。AI 代為摘要、但使用者尚未親自消化的原料，不走上面三條路徑——預摘要留 Inbox 當待讀佇列、保留來源連結，等本人讀過再決定內化或刪除。AI 的摘要 ≠ 使用者的理解，不可當成已內化的 Card。

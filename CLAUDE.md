...
- **路徑契約**：分兩種——
  ...
  - **其他 repo-local skill（vault-check / vault-distill / vault-youtube-sync / vault-updates-sync / vault-reddit-daily）**：cwd 必為 repo root，用 `content/...` 相對路徑直接讀寫；不依賴 env。前置作業會先驗 `test -f content/master-index.md`，cwd 不在 repo root 即中止

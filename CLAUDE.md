...
討論 vault 內容時，可順手點出成長面的觀察——某概念散在多頁卻沒專屬 Card、兩篇主題相關卻沒互連、某主題有明顯資料空缺。**只在討論中浮現、只提議不自動執行；agent 不主動定期掃描整個 vault 找這些。** 與「查詢回存」同源：偵測到 → 提醒，使用者拍板才動。

與 `/vault-lint` 的界線：**結構問題**（孤立頁、死連結、tag 漂移、缺欄位）走 `vault-lint` 主動掃，deterministic、可隨時跑；**成長面**（概念缺口、該連沒連、資料空缺）不進 vault-lint、不自動掃，只在討論時帶出。這是刻意的——成長判斷需讀正文語意、帶不確定性，不該拖慢日常結構快掃。

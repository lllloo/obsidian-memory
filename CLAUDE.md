...
- 工具更新同步（官方 changelog / GitHub releases / issues / discussions）→ 讀 `.agents/skills/vault-updates-sync/SKILL.md`
...
| `.claude/skills/vault-updates-sync/` | Skill | —        | 官方 changelog / GitHub releases / issues / discussions 高信任更新同步 |
...
- **路徑契約**：分兩種——
  ...
  - **其他 repo-local skill（vault-check / vault-topic-moc / vault-youtube-sync / vault-updates-sync / vault-reddit-daily）**：cwd 必為 repo root，用 `content/...` 相對路徑直接讀寫；不依賴 env。前置作業會先驗 `test -f content/master-index.md`，cwd 不在 repo root 即中止

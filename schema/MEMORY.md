...
- 2026-07-10 七線全面審核結案：README 對齊（批次 A）與強度標註/措辭對齊（批次 B）已修；**倉庫衛生兩項使用者拍板不動、勿重複提議**——`.gitignore` 不補 `.env`/金鑰類規則、`.obsidian/plugins/obsidian-git/` bundle 維持 tracked（換機開箱即用優先）。
